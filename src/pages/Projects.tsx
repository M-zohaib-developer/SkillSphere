...
import { Plus, Calendar, Edit, Trash2, Eye, Clock, Target, CheckCircle } from 'lucide-react';
...
const Projects: React.FC = () => {
  ...
  const [projects, setProjects] = useState<Project[]>([
    {
      ...
      description: 'A full-featured online store built with React and Node.js, featuring user authentication, payment processing, and inventory management.',
      status: 'in-progress',
    ...
    },
    {
      ...
      description: 'Interactive dashboard for analyzing sales data with charts, graphs, and real-time updates.',
      status: 'paused',
    ...
    },
    {
      ...
      description: 'Personal portfolio website showcasing my projects and skills with modern design and animations.',
      status: 'completed',
    ...
    },
    {
      ...
      description: 'Predictive model for customer behavior analysis using various ML algorithms and techniques.',
      status: 'in-progress',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({ title: '', description: '', status: 'in-progress', dueDate: '', tags: '' });
    setShowModal(false);
    setIsLoading(false);
  };
  ...
  const getStatusIcon = (status: Project['status']) => {
    switch (status) {
      ...
      case 'paused': return <Target className="h-4 w-4" />;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            ...
            { label: 'In Progress', value: projects.filter(p => p.status === 'in-progress').length, color: 'text-yellow-600' },
          ].map((stat, index) => (
            <div
              key={index}
              className={`${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} p-6 rounded-lg shadow-sm border`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <div
              key={project.id}
              className={`${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg shadow-sm border hover:shadow-lg transition-shadow duration-200`}
            >
              {/* Project Image */}
              {project.thumbnail && (
              ...
              <div className="p-6">
                ...
                {/* Dates */}
                <div className={`flex items-center justify-between text-sm mb-4 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                <div className="flex space-x-2">
                  <button className={`flex items-center space-x-1 px-3 py-2 text-sm rounded-md transition-colors duration-200 ${
                    theme === 'dark'
